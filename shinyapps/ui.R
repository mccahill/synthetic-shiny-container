...
ui <- shinyUI(fluidPage(
  ...
  wellPanel( 
    HTML("<h4>request remote processing</h4>"),
    htmlOutput("userSpecifiedJobDetail"),
    actionButton("remoteSubmitButton", "submit for remote processing"),
  ...
  wellPanel(
    ...
    fluidRow( column(1, HTML("<h4>design<br>the<br>model</h4>") ), 
              column(11, 
                     actionButton("goButton", "plot residuals"),
                     textInput("userSpecifiedModel", "")
                     #textOutput("userSpecifiedModel")
              )
    ),
    fluidRow(
